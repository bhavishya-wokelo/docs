...
### Exporting scored buyers to CSV

Retrieve results and export the AI-scored subset to a CSV for further analysis or client delivery.
...
scored = [b for b in buyers if b.get("Overall Score")]

with open("buyer_universe.csv", "w", newline="") as f:
    fields = [
        "Name", "Ticker", "HQ Country", "Buyer Type",
        "Revenue", "Market Cap",
        "Overall Score", "Deal Feasibility Score",
        "Product Synergy Score", "Deal Precedent Score", "Synergy Potential Score",
        "Commentary"
    ]
...
print(f"Exported {len(scored)} scored buyers to buyer_universe.csv")
```
