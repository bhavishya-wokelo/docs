...
### Exporting the funding dataset to CSV

Export the full records to CSV for CRM upload, investor outreach lists, or pipeline analysis.
...
fields = [
    ...
    "HQ City", "HQ Country", "HQ Continent",
    "Founded", "Product Category", "Core Offering",
    "Employees (Crunchbase)", "Employees (LinkedIn)",
    "Funding Stage", "Funding Date", "Funding Amount ($M)"
]

# Flatten list fields for CSV
def flatten(record):
    flat = {k: record.get(k) for k in fields}
    flat["Lead Investor(s)"] = "; ".join(record.get("Lead Investor(s)") or [])
    flat["Other Investor(s)"] = "; ".join(record.get("Other Investor(s)") or [])
    return flat

csv_fields = fields + ["Lead Investor(s)", "Other Investor(s)"]

with open("fundraise_activity.csv", "w", newline="") as f:
    writer = csv.DictWriter(f, fieldnames=csv_fields, extrasaction="ignore")
    writer.writeheader()
    writer.writerows(flatten(r) for r in records)

print(f"Exported {len(records)} funding records to fundraise_activity.csv")
```
...
df["Amount ($M)"] = df["Funding Amount ($M)"] / 1e6

# Funding activity by year
df["Year"] = pd.to_datetime(df["Funding Date"], errors="coerce").dt.year
print(df.groupby("Year")["Amount ($M)"].agg(["count", "sum"]).rename(
    columns={"count": "Rounds", "sum": "Total ($M)"}
))

# Top product categories by round count
print(df["Product Category"].value_counts().head(10))

# Stage breakdown
print(df["Funding Stage"].value_counts())

# Deduplicated company count (one row per company)
unique_companies = df.drop_duplicates(subset="Permalink")
